...
const Counters = () => {
    ...
    return (<>
            <counter_1.Counter onClick={() => setTooltip(1)} width={74} className={styles_module_scss_1.default.__sector} icon={(<_icons_1.IconSector />)} text={String(zone_1.zoneModel.selectors.useZoneSectors())}>
                {tooltip === 1 ? <ui_1.Tooltip pos="right" message="Все завоеванные территории"/> : <></>}
            </counter_1.Counter>

            

            <counter_1.Counter onClick={() => setTooltip(3)} width={74} className={styles_module_scss_1.default.__coin} icon={(<_icons_1.IconCoin />)} text={String(zone_1.zoneModel.selectors.useZoneCoins())}>

                {tooltip === 3 ? <ui_1.Tooltip pos="right" message="Монеты"/> : <></>}
            </counter_1.Counter>

            <counter_1.Counter onClick={() => setTooltip(4)} width={74} className={styles_module_scss_1.default.__sapphire} icon={(<_icons_1.IconSapphire />)} text={String(zone_1.zoneModel.selectors.useZoneRubies())}>

                {tooltip === 4 ? <ui_1.Tooltip pos="right" message="Кристаллы"/> : <></>}
            </counter_1.Counter>

            <counter_progress_1.CounterProgress onClick={() => setTooltip(5)} className={styles_module_scss_1.default.__experience} icon={(<_icons_1.IconExperience />)} progress={progressExp} color="#C163E0" counter={rankExp} width={73}>
                {tooltip === 5 ? <ui_1.Tooltip pos="left" message="Опыт завоеваний"/> : <></>}
            </counter_progress_1.CounterProgress>

            <counter_progress_1.CounterProgress onClick={() => setTooltip(6)} className={styles_module_scss_1.default.__zone} icon={(<_icons_1.IconZone />)} progress={zoneProgress} color="#C163E0" name={`Зона: ${zoneLevel} ур`} counter={zoneSects} width={73}>
